...
<?
$gostElements = Array();
$arSortGost = array("SORT" => "ASC");
$arFilterGost = array("IBLOCK_ID" => 15);
$arSelectGost = array("ID", "CODE", "NAME");
$itemsListGost = CIBlockElement::GetList($arSortGost, $arFilterGost, false, false, $arSelectGost);
if ($itemsListGost->SelectedRowsCount() > 0) {
  while ($itemGost = $itemsListGost->Fetch()) {
    $gostElements[$itemGost['NAME']] = $itemGost['CODE'];
  }
}
?>
...
<div class="row">
    ...
    ?>
        ...
        <?}?>
        <?include_once 'properties_list.php'; // блок характеристики добавлен справа от картинки товара?>
    </div>
    <div class="clearfix"></div>
</div>
