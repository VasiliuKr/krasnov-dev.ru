        ...
        <tr data-offer-id="<?=$offer["ID"]?>" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
            ...
            <td class="basket-name">
                <?if($bxr_use_links_sku == "Y"):?>
                    <a href="<?=$offer["DETAIL_PAGE_URL"];?>" class="bxr-font-hover-light" itemprop="sku">
                        <?/*=$offer["NAME"]*/?>
                        <?=substr($offer["NAME"], strlen($arResult["NAME"]))?>
                    </a>
                <?else:?>
            ...
            <td class="basket-price bxr-format-price hidden-xs">
				...
				if ($priceNameShow && count($offer["PRICES"]) > 1) {?>
                                ...
                                ?>
                                <span class="bxr-market-current-price bxr-market-format-price"<?=($arItemIDs['PRICE'] > 0)?' id="offer_'.$offer["ID"].'_'.$arItemIDs['PRICE'].'"':''; ?>><?=CurrencyFormat($arPrice['DISCOUNT_VALUE'], $arPrice['CURRENCY'])." / "?><?=$arResult["CATALOG_MEASURE_NAME"]?></span>
                                <? //--old price--
            ...
            <td class="basket-line-qty">
                 <span class="bxr-market-current-price bxr-market-format-price hidden-lg hidden-md hidden-sm" id="qty_<?=$arItemIDs['PRICE'].'_'.$offer["ID"];?>"><?=CurrencyFormat($arPrice['DISCOUNT_VALUE'], $arPrice['CURRENCY'])." / "?><?=$arResult["CATALOG_MEASURE_NAME"]?></span>
