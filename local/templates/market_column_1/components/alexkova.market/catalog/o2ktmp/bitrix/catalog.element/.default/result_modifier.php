...
if( count($arResult['SECTION']['PATH']) > 1 ){ //если в url есть подразделы, делаем редирект
	header("Location: ".$arResult['SECTION']['PATH'][0]['SECTION_PAGE_URL'].$arResult['CODE'].'/',false,301);
	exit;
}else{ //добавляем недостающие разделы в цепочку навигации
	$nav = CIBlockSection::GetNavChain($arResult['IBLOCK_ID'], $arResult['IBLOCK_SECTION_ID'], array('ID','NAME','SECTION_PAGE_URL'));
	while( $navRes = $nav->GetNext() ){
		if( $navRes['ID'] != $arResult['SECTION']['PATH'][0]['ID'] )
			$arResult['SECTION']['PATH'][] = $navRes;
	}
}
...
//			Получаем картинку текущего раздела
$sectionCur = CIBlockSection::GetByID($arResult["IBLOCK_SECTION_ID"]);
$arSectionCur = $sectionCur->GetNext();
$arResult["SECTION_IMG"] = CFile::GetFileArray($arSectionCur["PICTURE"]);
...
//			Если картинки товара нет, то показываем в карточке товаров картинку раздела, если она есть
if (!empty($arResult["SECTION_IMG"]) && $arResult["DETAIL_PICTURE"] == false) {
	$arResult['MORE_PHOTO'][0]['SRC'] = $arResult["SECTION_IMG"]["SRC"];
}
