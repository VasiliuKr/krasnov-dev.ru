...
if (!$arParams["DEFAULT_CATALOG_VIEW"]) {
	$arParams["DEFAULT_CATALOG_VIEW"] = "TABLE";
}
...
$sort = array_key_exists("sort", $_GET) && array_key_exists(ToUpper($_GET["sort"]), $arResult["SORT_PROPS"]) ? $arResult["SORT_PROPS"][ToUpper($_GET["sort"])][0] : 'sort';
$sort_order = array_key_exists("order", $_GET) && in_array(ToLower($_GET["order"]), Array("asc", "desc")) ? ToLower($_GET["order"]) : 'asc';
